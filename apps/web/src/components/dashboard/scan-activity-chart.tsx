...
import { useModerationActivity } from "@/lib/queries";
...
function ScanActivityChartSkeleton() {
  return (
    <div
      role="status"
      aria-live="polite"
    ...
    >
      <span className="sr-only">Loading scan activity</span>
...
export function ScanActivityChart() {
  const { data: activity, isLoading, error, refetch } = useModerationActivity(7);
  ...
  return (
    <Card>
      <CardHeader className="border-b border-border py-4 px-5">
        <CardTitle className="card-title">Scan Activity</CardTitle>
      ...
      <CardContent className="p-5">
        {isLoading ? (
          ...
          <EmptyState
            icon={BarChart3}
            title="No scans yet"
            description="Run a scan to see moderation activity trends here."
          />
        ) : (
            ...
            <BarChart data={data} margin={{ top: 8, right: 4, left: -16, bottom: 0 }}>
              <defs>
                <linearGradient id="scans-fill" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="var(--color-scans)" stopOpacity={0.95} />
                  <stop offset="100%" stopColor="var(--color-scans)" stopOpacity={0.55} />
                </linearGradient>
              </defs>
              <CartesianGrid
                vertical={false}
