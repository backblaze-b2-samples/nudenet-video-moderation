...
/**
 * Upload a video and run the moderation pipeline synchronously. Progress
 * reflects the UPLOAD only — the server-side scan runs after the bytes land.
 */
export function createModerationJob(
  file: File,
  options: ModerationJobOptions = {},
  ...
  return new Promise((resolve, reject) => {
    ...
    if (options.sampleMode) formData.append("sample_mode", options.sampleMode);
    if (options.sampleFps !== undefined)
      formData.append("sample_fps", String(options.sampleFps));
    if (options.threshold !== undefined)
      formData.append("threshold", String(options.threshold));
    if (options.violationLabels)
      formData.append("violation_labels", options.violationLabels);
    ...
    xhr.addEventListener("load", () => {
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        try {
          ...
          reject(new ApiError(body.detail || `Scan failed: ${xhr.status}`, xhr.status));
        } catch {
          reject(new ApiError(`Scan failed: ${xhr.status}`, xhr.status));
        }
      }
    });
    ...
    xhr.addEventListener("abort", () =>
      reject(new ApiError("Scan aborted", 0)),
    );

    xhr.open("POST", `${API_BASE}/moderation/jobs`);
    xhr.send(formData);
  });
}

export async function getModerationReports(limit = 100) {
  return apiFetch<ManifestEntry[]>(`/moderation/reports?limit=${limit}`);
}

export async function getModerationReport(videoId: string) {
  return apiFetch<ModerationReport>(
    `/moderation/reports/${encodeURIComponent(videoId)}`
  );
}

export async function getModerationStats() {
  return apiFetch<ModerationStats>("/moderation/stats");
}

export async function getModerationActivity(days = 7) {
  return apiFetch<DailyScanCount[]>(`/moderation/stats/activity?days=${days}`);
}

export async function updateModerationReview(videoId: string, body: ReviewUpdate) {
  return apiFetch<ModerationReport>(
    `/moderation/reports/${encodeURIComponent(videoId)}`,
    {
      method: "PATCH",
...
export async function rerunModerationReport(videoId: string, threshold?: number) {
  return apiFetch<ModerationReport>(
    `/moderation/reports/${encodeURIComponent(videoId)}/rerun`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(
        threshold !== undefined
          ? { review_decision: "pending", review_notes: "", threshold }
          : {}
      ),
    }
  );
...
export async function deleteModerationReport(videoId: string) {
  return apiFetch<{ deleted: boolean; video_id: string }>(
    `/moderation/reports/${encodeURIComponent(videoId)}`,
    { method: "DELETE" }
  );
}
