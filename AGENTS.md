...
This app is the **NudeNet Video Moderation** sample, built on the
vibe-coding-starter-kit shell. The following pieces are load-bearing — keep them.
...
- **Bucket Explorer.** `/files` route, `apps/web/src/app/files/`, and `apps/web/src/components/files/`. This is the full-bucket browser (raw uploads + all moderation artifacts). The Files sidebar entry stays.
- The sidebar nav (Dashboard, New Scan, Review Queue, Files, Settings, plus the Design System utility link).

**Moderation surface (the app's reason to exist)**
- **Pipeline.** `services/api/app/service/{frames,classifier,moderation,moderation_store}.py`. NudeNet is a local ONNX detector — heavy ML imports stay LAZY (inside functions) so structure/no-network tests run offline.
- **Routes.** `services/api/app/runtime/moderation.py` (`/moderation/*`).
- **Frontend.** Dashboard (`/`), New Scan (`/jobs/new`), Review Queue (`/jobs`), report detail (`/jobs/[videoId]`). The moderation report is the primary entity with full create/read/edit/run/delete.
- New aggregations must flow through `runtime → service → repo` layering and be exposed via TanStack Query hooks in `apps/web/src/lib/queries.ts` — no bare `useEffect + fetch`.

**Why this contract exists**
- The UI kit and Bucket Explorer are the reusable B2-backed scaffolding. The moderation pipeline + report lifecycle is what makes this app distinct.
