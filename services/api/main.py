...
from app.runtime import files, health, metrics, moderation  # noqa: E402
...
# endpoint is DERIVED from B2_REGION (settings.b2_endpoint), so the region is
# what we validate here, not a separate endpoint string.
...
app = FastAPI(
    title="NudeNet Video Moderation API",
    description="Self-hosted NSFW video moderation pipeline backed by Backblaze B2",
    version="0.1.0",
    lifespan=lifespan,
)
