...
def get_s3_client():
    # The endpoint is derived from B2_REGION (see settings.b2_endpoint) and the
    # region is passed explicitly so botocore signs with the correct S3v4
    # region. No region string is hardcoded here.
    return boto3.client(
        "s3",
        ...
        config=Config(
            ...
            user_agent_extra="b2-nudenet-video-moderation",
        ),
    )
...
def get_object_bytes(key: str) -> bytes | None:
    """Read an object's bytes from B2. Returns None on 404."""
    client = get_s3_client()
    try:
...
def list_files(prefix: str = "", max_keys: int = 1000) -> list[FileMetadata]:
    """List files from B2 (paginated). Raises RuntimeError on S3 failure."""
    client = get_s3_client()
    files: list[FileMetadata] = []
    kwargs: dict = {"Bucket": settings.b2_bucket_name, "MaxKeys": min(max_keys, 1000)}
    if prefix:
...
def list_keys(prefix: str) -> list[str]:
    """Return every object key under a prefix (paginated)."""
    client = get_s3_client()
    keys: list[str] = []
